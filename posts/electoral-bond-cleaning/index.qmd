...
10. Looking at gaps using both Bond Number and prefix

```{r}
anti_join(df_buyer, df_party, by = c(Bond.Number,Prefix))

# 18871-130 = 18741 linked from buyer to party
# We have merged the party data frame into df_buyer so as to keep all the names of buyers and amounts, even if we do not have data on the redemption

anti_join(df_party, df_buyer, by = c(Bond.Number,Prefix))

# We will use this data frame for the unknown buyer set 
# 20421 - 1680 = 18941

# there is a gap of 200 between both 18941-18741 = 200
```
...
df_unknownBuyer <- anti_join(df_party, df_buyer, by = c(Bond.Number,Prefix))


df_unknownBuyer |> 
    group_by(Bond.Number) |> 
    arrange(desc(Bond.Number)) |> 
    filter(n()>1)
...
1.  Review of names and dates

    ```{r}
    #| label: variable name cleaning

    var_names <- ls(df_merge_buyerParty)

    # names(df_merge_buyerParty)
    ```
...
colnames(df_merge_buyerParty)

df_merge_buyerParty <- df_merge_buyerParty %>% 
    rename(sr_no = Sr.No.,
           ref_urn = Reference.No...URN.,
           date_journal = Journal.Date,
           date_purchase = Date.of.Purchase,
           date_expiry = Date.of.Expiry,
           name_buyer  = Name.of.the.Purchaser,
           denominations = Denominations.x,
           name_party  = Name.of.the.Political.Party)



names(df_merge_buyerParty) <-  tolower(names(df_merge_buyerParty))

names(df_merge_buyerParty) <- sub("\\.", "_", names(df_merge_buyerParty))

colnames(df_merge_buyerParty)

```

### Consolidating and cleaning of buyer names

1.  Consolidation buyer - party -dates

2.  Cleaning names

```{r}
#| label: saving for use with sublime text

df_merge_buyerParty |> 
    select(name_buyer) |> 
    unique() |> 
    capture.output() |> 
    writeLines("./data/processed/namesBuyers.txt")
